package bestbuydatasupply;

import base.CommonAPI;
import org.openqa.selenium.InvalidArgumentException;
import org.openqa.selenium.WebDriver;
import org.openqa.selenium.support.PageFactory;
import reporting.TestLogger;
import bestbuyfetch.FetchBestBuySteps;

import java.io.IOException;

public class BestBuyFunctionality {

    BestBuyLandingPage landingPage = null;
    BestBuyLoginPage logInPage = null;


    public void logIn(WebDriver driver){
        TestLogger.log(getClass().getSimpleName() + ": " + CommonAPI.convertToString(new Object(){}.getClass().getEnclosingMethod().getName()));
        landingPage = PageFactory.initElements(driver, BestBuyLandingPage.class);
        logInPage = PageFactory.initElements(driver, BestBuyLoginPage.class);
        //landingPage.clickOnLogIn();
        //logInPage.enterEmailAddress("abc123@gmail.com");
        //logInPage.enterPassword("abc123");
        //logInPage.clickOnLogInSubmitButton();
    }

    public void runAllTheFeatureTest(WebDriver driver) throws InterruptedException, IOException {
        TestLogger.log(getClass().getSimpleName() + ": " + CommonAPI.convertToString(new Object(){}.getClass().getEnclosingMethod().getName()));
        FetchBestBuySteps fetchTheSteps = new FetchBestBuySteps();
        //String[] featureSteps = fetchTheSteps.getDataFromExcelFile();
        //for (int i=1; i<featureSteps.length; i++){
        //    select(featureSteps[i], driver);
        }
    //}
    public void select(String featureName, WebDriver driver)throws InterruptedException, IOException {
        TestLogger.log(getClass().getSimpleName() + ": " + CommonAPI.convertToString(new Object(){}.getClass().getEnclosingMethod().getName()));
        switch(featureName){
            case "signUp":
                logIn(driver);
                break;
            default:
                throw new InvalidArgumentException("Invalid features choice");
        }
    }
}